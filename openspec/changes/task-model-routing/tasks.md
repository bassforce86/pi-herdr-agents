...
- [ ] 6.2 Sync bundled role prompts in `agents/` and `skills/orchestrate/SKILL.md` guidance that reference model selection (at minimum `adversarial-reviewer.md`) with the curated-shortlist contract and the review exact-ID rule, and verify `npm run format:check` and `test/package-skill.test.js` pass
